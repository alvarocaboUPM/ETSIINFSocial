...
public class ETSIINFSocialSkeleton {
    ...
    public es.upm.etsiinf.sos.LoginResponse login(
        ...
        if (adminLoggedIn || res) {
            // Si el usuario admin está autenticado o el nombre de usuario y contraseña son
            // válidos
            // según el servicio UPMAuthenticationAuthorization, la operación de login tiene
            // éxito.
    ...
    public es.upm.etsiinf.sos.RemoveFriendResponse removeFriend(
        ...
        if (!isFriend(username)) {
            System.out.println("El usuario " + userID.getName() + " no tiene añadido como amigo al usuario "
                    + username.getUsername());
            response.setResponse(false);
            res.set_return(response);
            return res;
